class GPT2_ModelConfig:
    ...
    def load_model_config(self, model_name, context_length = 1024, drop_rate = 0.0):

        updated_config = self.base_config.copy()
        updated_config.update(self.config_dict[model_name])
        updated_config.update({'qkv_bias':True, 'context_length':context_length, 'dropout': drop_rate})
